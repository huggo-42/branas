...
class PostgreSQLConnection implements Connection {
    ...
    constructor() {
        // this.pgp = pgp()("postgress://postgress:123456@localhost:5432/app")
        this.pgp = pgp()("postgres://nihil:123456@localhost:5432/app");
    }

    query(statements: string, params: any): Promise<any> {
        return this.pgp.query(statements, params);
    }
...
class ORM {
    ...
    async save(entity: Entity) {
        ...
        const params = entity.columns.map((_, index) => `$${index + 1}`).join(",");
        const values = entity.columns.map(column => entity[column.property]).join(",");
